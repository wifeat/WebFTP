...
//ZIP解压
	...
	$name    = u2g(trim($_POST['name']));
	$remove  = (int)trim($_POST['remove']);
	$unzippath = u2g(trim($_POST['unzippath']));
	if(file_exists($path.$name) && is_file($path.$name)){
		...
		$statusCode = 300;
		$message = 'Sorry,未知错误,无法解压:'.$path.$name.'文件！';
	}
	return_json($statusCode, $message);
//ZIP压缩
}elseif('zip' == $module){
		...
		if('file' == $action && is_file($files[0])){
		    $zipname = $path.pathinfo($files[0], PATHINFO_BASENAME).'-'.time().'.zip';
		}elseif('dir' == $action && is_dir($files[0])){
		    $zipname = $path.basename($files[0]).'-'.time().'.zip';
		}else{
		    $zipname = $path.basename($_POST['path']).'-'.time().'.zip';
		}
		include(INC_ROOT.'PclZip.class.php');
		$Zip = new PclZip($zipname);
		if(!file_exists($zipname) && $Zip->create($files, PCLZIP_OPT_REMOVE_PATH, $path)){
			$statusCode = 200;
		    $list = $Zip->listContent();
		    if($list){
				$fold = 0; $fil = 0; $tot_comp = 0; $tot_uncomp = 0;
			...
			$statusCode = 300;
			if(file_exists($zipname)){$error = g2u(basename($zipname)).'已经存在！';}else{$error = $Zip->errorInfo(true);}
			$message    = '<font color="blue">压缩失败：</font><font color="red">'.$error.'</font><br />';
			$message   .= '<font color="green">执行耗时：</font><font color="red">'.G('_run_start','_run_end',6).' 秒</font><br />';
		}
		return_json($statusCode,$message);
