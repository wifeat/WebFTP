...
class File
{
	...
	static function chmod($path, $chmod, &$info, &$err){
		...
		if (self::setChmod($path,$chmod)){$info['dirs']++;}
		return $info['dirs'];
	}
	...
	static function move1($from,$to,$cover,$cut,&$coverfiles,&$info) {
		if(!is_dir($to)){mk_dir($to,0755);$info['dirs']++;}
		$dirs = $files = array();
		...
		if(file_exists($from)){
			...
			if(file_exists($to) && false == $cover){
				$coverfiles[]=$to;
					...
					$cut && unlink($to);
					if(self::move3($from, $to, $cut)){$info['files']++;}
			}else{
				if(self::move3($from,$to,$cut)){$info['files']++;}
			}
		}
