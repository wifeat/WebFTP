...
CodePress = {
		...
		if((evt.ctrlKey || evt.metaKey) && evt.shiftKey && charCode!=90)  { // shortcuts = ctrl||appleKey+shift+key!=z(undo)
			...
			evt.preventDefault();
		 	...
		 	top.setTimeout(function(){
				CodePress.syntaxHighlight('php');},100);
		}
	...
	getCode : function() {
		...
		//var myeditor = CodePress.getEditor();
		//var code = myeditor.innerHTML;
		var code = '';
		var size = document.getElementsByTagName('pre').length;
		for(var i=0; i<size; i++){
			code += document.getElementsByTagName('pre')[i].innerHTML+'<br>';
		}
